export type SourcePlatform = "whop" | "vyro" | "mrktplce" | "reach_cat" | "manual";
...
export function calculateWorkerPayout(sourcePayoutPer1k: number, sourcePlatform?: SourcePlatform) {
  if (sourcePlatform === "reach_cat") {
    if (sourcePayoutPer1k >= 4) return 0.5;
    if (sourcePayoutPer1k >= 2) return 0.35;
    return 0.25;
  }
...
export function validateImportedCampaign(input: ImportedCampaignInput): CampaignFilterResult {
  if (input.sourcePlatform === "reach_cat") {
    if (input.budgetPctRemaining < 40) {
      return {
        allowed: false,
        reason: "Reach.cat campaign budget is below 40% remaining and may run out before views verify.",
...
export function inferNiche(text: string) {
  ...
  if (["fitness", "health", "workout", "mobility", "gym"].some((term) => normalized.includes(term))) return "fitness";
  if (["shop", "ecommerce", "product", "store", "amazon", "tiktok shop"].some((term) => normalized.includes(term))) return "ecommerce";
