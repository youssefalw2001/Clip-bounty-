    ...
    <main className="relative min-h-screen overflow-hidden bg-[#050505] text-stone-50">
      ...
      <nav className="relative z-10 border-b border-white/10 bg-black/45 px-5 py-4 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4">
          ...
          <Link href="/console" className="rounded-full border border-emerald-300/30 bg-emerald-300/10 px-4 py-2 text-xs font-bold uppercase tracking-widest text-emerald-200 shadow-lg shadow-emerald-950/20">
            App Console
          </Link>
        </div>
        ...
        <div className="mx-auto mt-4 max-w-6xl space-y-3">
          {navGroups.map((group) => (
            <div key={group.label} className="flex items-center gap-2 overflow-x-auto pb-1 text-sm text-stone-300">
              <span className="shrink-0 rounded-full border border-white/10 bg-black/30 px-3 py-2 text-xs font-black uppercase tracking-widest text-stone-500">
                {group.label}
              </span>
              {group.items.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="shrink-0 rounded-full border border-white/10 bg-white/5 px-4 py-2 font-semibold transition hover:bg-white/10 hover:text-white"
                >
                  {item.label}
                </Link>
              ))}
