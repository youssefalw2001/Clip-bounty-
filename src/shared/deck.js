/**
 * Canonical prompt deck — the single source of truth.
 *
 * Plain ESM so that BOTH the Vite/TS client and the Node server can import it.
 * The server is authoritative over which prompt a match gets, so if this data
 * lived only in the TypeScript client the two would drift the moment anyone
 * edited a prompt. `src/lib/prompts.ts` re-exports this with types attached.
 *
 * Design rules for every prompt here:
 ...
 *     content, so escalating gains the player nothing.
...
export const PROMPTS = [
  ...
  // ---- EVIDENCE: photograph your own screen. brutal, very shareable ----
...
/**
 * The alternative to revealing. A real choice is what keeps this a game the
 * loser is playing rather than something being done to them — and the forfeit
 * is frequently funnier than the photo would have been.
 */
...
/** Match rules, shared so client and server can never disagree. */
export const RULES = {
  FOUL: 99_999,
  ROUNDS_TO_WIN: 2,
  TOTAL_ROUNDS: 3,
  /** Faster than this is not humanly possible — treated as a jumped gun. */
  MIN_HUMAN_MS: 80,
  /** How long a player has to react before it's scored as a foul. */
  TAP_TIMEOUT_MS: 5000,
  CAPTURE_SECONDS: 15,
}
